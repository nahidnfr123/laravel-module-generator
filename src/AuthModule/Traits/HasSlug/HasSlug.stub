...
trait HasSlug
{
    ...
    public function resolveRouteBinding($value, $field = null): Model|Collection|static
    {
        ...
        if ($this->slugOptions->replaceRouteKey) {
            return self::where($this->slugOptions->slugField, $value)
                ->firstOrFail();
        }
        if (is_numeric($value)) {
    ...
    protected function generateNonUniqueSlug(): string
    {
        ...
        if (! empty($this->$slugField) && $this->hasCustomSlugBeenUsed()) {
    ...
    public static function findBySlug(string $slug, array $columns = ['*'])
    {
        ...
        $field = in_array(HasTranslatableSlug::class, class_uses_recursive(static::class), false)
