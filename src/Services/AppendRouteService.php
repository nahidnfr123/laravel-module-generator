...
class AppendRouteService
{
    ...
    public function appendRoute(string $tableName, string $controllerClass): void
    {
        ...
        if ($isApi) {
            // API routes
            $routeLine = "Route::apiResource('{$tableName}', \\{$appNamespace}Http\\Controllers\\{$controllerClass}::class);";
            $routesPath = base_path('routes/api.php');
            $routeType = 'API';
            $defaultContent = $this->getDefaultApiContent();
        } else {
            // Web routes - use resource instead of apiResource
            $routeLine = "Route::resource('{$tableName}', \\{$appNamespace}Http\\Controllers\\{$controllerClass}::class);";
